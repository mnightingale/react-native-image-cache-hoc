...
describe('lib/FileSystem', function () {
  ...
  it('#exists mocked as true.', () => {
    ...
    return expect(fileSystem.exists('abitrary-file.jpg')).resolves.toBe(true);
  });
  ...
  it('#getLocalFilePathFromUrl should return local filepath if it exists on local fs in permanent dir.', () => {
    const RNFS = require('react-native-fs');
    RNFS.exists
      .mockResolvedValueOnce(true) // mock exist in local permanent dir
  ...
  it('#getLocalFilePathFromUrl should return local filepath if it exists on local fs in cache dir.', () => {
    ...
    RNFS.exists
      ...
      .mockResolvedValueOnce(true) // mock exist in local cache dir
      .mockResolvedValue(true);
  ...
  it('#getLocalFilePathFromUrl should download file and write to disk (default to cache dir) if it does not exist on local fs.', () => {
    ...
    ).resolves.toEqual('/base/file/path/react-native-image-cache-hoc/cache/cd7d2199cd8e088cdfd9c99fc6359666adc36289.png');
  });
    ...
    return fileSystem
      ...
      .then((result) => {
        result.should.deepEqual({
          path: '/base/file/path/react-native-image-cache-hoc/cache/cd7d2199cd8e088cdfd9c99fc6359666adc36289.png',
  ...
  it('#unlink should work as expected for valid paths.', () => {
    ...
    return expect(fileSystem.unlink(validPath)).resolves.toBe(true);
  });
});
