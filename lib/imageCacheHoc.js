...
export default function imageCacheHoc(Image, options = {}) {
  ...
  return class extends React.PureComponent {
     ...
     * be sure to use a queue and limit concurrency so your app performance does not suffer.
     *
     * @param url {String} - url of local file.
     * @param url {String} - url of file.
     * @param permanent {Boolean} - whether the file should be saved to the tmp or permanent cache directory.
     * @param move {Boolean} - whether the file should be moved.
     * @returns {Promise} promise that resolves to an object that contains cached file info.
     */
    static async cacheLocalFile(local, url, permanent = false, move = false) {
      const fileSystem = FileSystemFactory(options.cachePruneTriggerLimit, options.fileDirName);
      const localFilePath = await fileSystem.getLocalFilePathFromUrlLocal(
        local,
        url,
        permanent,
        move
    ...
    async componentDidMount() {
      ...
      try {
        ...
        this.setState({ remoteFilePath: url });
    ...
    async componentDidUpdate(prevProps) {
      ...
      this.setState({ remoteFilePath: nextUrl });
    ...
    async _loadImage(url) {
      ...
      // Check if we are still interested in the url
      if (this.state.remoteFilePath !== url) {
        return;
      }
      ...
      if (this._isMounted) {
        ...
        if (!this.InvalidUrl && this.props.onLoadFinished) {
          if (Platform.OS === 'android') {
            await Image.getSize('file://' + localFilePath, (width, height) =>
              this.props.onLoadFinished(width, height)
            );
          } else {
            await Image.getSize(localFilePath, (width, height) =>
              this.props.onLoadFinished(width, height)
            );
          }
        }
